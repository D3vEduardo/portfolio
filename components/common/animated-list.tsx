...
export function AnimatedList<T extends AnimatedListItem>({
  ...
  const containerClasses = `${className} ${horizontal ? "flex" : ""} ${
    horizontal && sameWidth ? "items-stretch w-full" : ""
  }`;
  ...
  return (
    <div ref={containerRef} className={containerClasses} id={id}>
      {items.map((item, index) => (
        <div
          key={item.id}
          className={`animated-list-item cursor-pointer ${
            sameWidth && !horizontal ? "w-full" : ""
